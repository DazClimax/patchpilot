...
PATCHPILOT_LOG_DIR="${PATCHPILOT_LOG_DIR:-$PATCHPILOT_DATA_DIR/logs}"
PATCHPILOT_LOG_FILE="${PATCHPILOT_LOG_FILE:-$PATCHPILOT_LOG_DIR/server.log}"
PATCHPILOT_BOOTSTRAP_PASSWORD_FILE="${PATCHPILOT_BOOTSTRAP_PASSWORD_FILE:-$PATCHPILOT_DATA_DIR/bootstrap-admin.txt}"

export PATCHPILOT_DATA_DIR PATCHPILOT_DB_PATH PATCHPILOT_ENV_FILE PATCHPILOT_SSL_DIR PATCHPILOT_LOG_DIR PATCHPILOT_LOG_FILE PATCHPILOT_BOOTSTRAP_PASSWORD_FILE
...
mkdir -p "$PATCHPILOT_DATA_DIR" "$PATCHPILOT_SSL_DIR" "$PATCHPILOT_LOG_DIR"
touch "$PATCHPILOT_ENV_FILE"
touch "$PATCHPILOT_LOG_FILE"
chmod 600 "$PATCHPILOT_ENV_FILE"
chmod 640 "$PATCHPILOT_LOG_FILE"
...
echo "[patchpilot-docker] Log file : $PATCHPILOT_LOG_FILE"
echo "[patchpilot-docker] UI port  : ${PORT}"
echo "[patchpilot-docker] Agent    : ${AGENT_PORT}"
if [ ! -f "$PATCHPILOT_DB_PATH" ] && [ -z "${PATCHPILOT_ADMIN_PASSWORD:-}" ]; then
  echo "[patchpilot-docker] Bootstrap credentials will be written to: $PATCHPILOT_BOOTSTRAP_PASSWORD_FILE"
fi
